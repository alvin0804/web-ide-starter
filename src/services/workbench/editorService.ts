...
export interface IEditorService extends Component<IEditor> {
   ...
   * @param callback 
   */
  onQucikCreate(callback: () => void): void;
  /**
  ...
  setDefaultHandlers(handlers: IEditorHandlerItem[]): void;
  /**
  ...
  getDefaultHandlers(): IEditorHandlerItem[];
  /**
  ...
  getHandlerValues(groupId: UniqueId, tabId: UniqueId): Record<string, unknown>;
  getHandlerById(groupId: UniqueId, tabId: UniqueId, handlerId: string): IEditorHandlerItem;
  setHandlerById(groupId: UniqueId, tabId: UniqueId, handlerId: string, handlerItem: IEditorHandlerItem): void;
  setHandlerLockStatus(groupId: UniqueId, tabId: UniqueId, handlerId: string, isLockStatus: boolean): void;

  // lockHanlderItem(handlerId: UniqueId): void;
}
@singleton()
export class EditorService
  ...
  protected defaultHandlers: IEditorHandlerItem[] = [];
  ...
  public getDefaultHandlers(): IEditorHandlerItem[] {
    return cloneDeep(this.defaultHandlers);
  }
  ...
  public setDefaultHandlers(handlers: IEditorHandlerItem[]): void {
  ...
  public updateTab(tab: IEditorTab, groupId?: UniqueId): IEditorTab {
    ...
    if (groupId) {
      const group: any = this.getGroupById(groupId);
      ...
      if (current?.activeTab === tab.id) {
        updatedTab = Object.assign((current as any)!.tab, tab);
      }
  ...
  public open<T>(tab: IEditorTab<T>, groupId?: UniqueId) {
    ...
    // 设置默认的handlers
    tab.handlers = cloneDeep(this.defaultHandlers);
  ...
  public getHandlerValues(groupId: UniqueId, tabId: UniqueId): Record<string, unknown> {
    const tab = this.getTabById(tabId, groupId);
    const handlerValues = tab.handlers
      .map((item: IEditorHandlerItem) => ({ [item.id]: item.value }))
      .reduce((previous: Record<string, string>, current: Record<string, string>) => ({ ...previous, ...current }), {})
    return handlerValues;
  }
  public getHandlerById(groupId: UniqueId, tabId: UniqueId, handlerId: string): IEditorHandlerItem | undefined {
    const tab = this.getTabById(tabId, groupId);
    const nextHandlers = tab.handlers?.concat() || [];
    const current = nextHandlers.find((item: IEditorHandlerItem) => item.id === handlerId);
    return current;
  }
  public setHandlerById(groupId: UniqueId, tabId: UniqueId, handlerId: string, handlerItem: IEditorHandlerItem): void {
    // const { current: currentGroup } = this.getState();

    // const group = this.getGroupById(groupId);
    const tab = cloneDeep(this.getTabById(tabId, groupId))
    const handlers = tab.handlers?.concat() || [];
    const nextHandlers = handlers.map((item: IEditorHandlerItem) => {
      if (item.id === handlerId) {
        return cloneDeep(handlerItem)
      }
      return item;
    })
    tab.handlers = cloneDeep(nextHandlers);
    this.updateTab(tab, groupId);
  ...
  public setHandlerLockStatus(groupId: UniqueId, tabId: UniqueId, handlerId: string, isLockStatus: boolean): void {
    const handler = this.getHandlerById(groupId, tabId, handlerId);
    handler.isLockStatus = isLockStatus;
    this.setHandlerById(groupId, tabId, handlerId, cloneDeep(handler));
  }
  ...
  public onQucikCreate(callback: () => void) {
    this.subscribe(EditorEvent.onQuickCreate, callback);
  }
}
