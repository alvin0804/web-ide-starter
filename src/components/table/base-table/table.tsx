...
import { distinctUntilChanged, filter, map, mapTo, pairwise, shareReplay, skip, startWith } from "rxjs/operators";
...
export class BaseTable extends Component<BaseTableProps, BaseTableState> {
  static defaultProps = {
    hasHeader: true,
    isStickyHeader: true,
    stickyTop: 0,
  ...
  private artTableWrapperRef = createRef<HTMLDivElement>()
  ...
  private renderTableBody(info: RenderInfo) {
    ...
    const tableBodyClassName = cx(Classes.tableBody, Classes.horizontalScrollContainer, {
      'no-scrollbar': footerDataSource !== undefined ? footerDataSource.length > 0 : false,
    })
  ...
  render() {
    ...
    const artTableWrapperClassName = cx(
      Classes.artTableWrapper,
      {
        ...
        'has-footer': footerDataSource !== undefined ? footerDataSource.length > 0 : false,
  ...
  private initSubscriptions() {
    ...
    const richVisibleRects$ = getRichVisibleRectsStream(
      ...
      this.props$.pipe(skip(1), mapTo('structure-may-change')),
    ...
    this.rootSubscription.add(
      combineLatest([
        ...
        this.props$.pipe(
          startWith(null),
          pairwise(),
          filter(([prevProps, props]) => prevProps == null || (!prevProps.isLoading && props.isLoading)),
        ),
      ]).subscribe(([clipRect]) => {
