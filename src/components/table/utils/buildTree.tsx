...
export default function buildTree<ID extends string, PID extends string, T extends { [key in ID | PID]: string }>(
  ...
  const ensure = (id: string) => {
    ...
    const wrapper: Wrapper = { id, parent: null as any, item: null as any, children: [] }
    wrapperMap.set(id, wrapper)
    return wrapper
  }
  for (const item of items) {
    const parentWrapper = ensure(item[parentIdProp]) as any;
    const itemWrapper = ensure(item[idProp]) as any;
